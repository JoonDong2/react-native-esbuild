...
function getCommands() {
  ...
  const commandNames = Object.values(projectCommands).map(
    ({ name }: any) => name
  );
...
const startCommand: any = cliCommands.find(({ name }: any) => name === 'start');
const bundleCommand: any = cliCommands.find(
  ({ name }: any) => name === 'bundle'
);
...
const bundleCommandOptions = [
  ...
  },
  {
    ...
    parse: (val: any) => path.resolve(val),
  },
  {
    name: '--stats <preset>',
    description:
...
module.exports = [
  {
    ...
    func: require('./bundle').bundle,
  },
  {
    name: 'start',
    options: startCommandOptions,
    description: startCommand.description,
    func: require('./start').start,
  },
];
