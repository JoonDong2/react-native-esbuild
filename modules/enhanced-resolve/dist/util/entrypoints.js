...
function conditionalMapping(conditionalMapping_, conditionNames) {
  ...
  loop: while (lookup.length > 0) {
    ...
    const conditionNameIndexMap = new Map(
      Array.from(conditionNames).map((item, index) => [item, index])
    );

    const sortedConditions = conditions.sort((x, y) => {
      const indexX = conditionNameIndexMap.has(x)
        ? conditionNameIndexMap.get(x)
        : Infinity;
      const indexY = conditionNameIndexMap.has(y)
        ? conditionNameIndexMap.get(y)
        : Infinity;
      return indexX - indexY;
    });

    for (let i = j; i < sortedConditions.length; i++) {
